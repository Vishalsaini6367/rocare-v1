        ...
        <div className="min-h-screen bg-slate-50">
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 animate-fade-in group">
                ...
                <div className="grid lg:grid-cols-2 gap-20">
                    ...
                    <div className="space-y-10 group/img">
                        <div className="bg-white p-16 rounded-[4rem] shadow-2xl border border-slate-100 relative overflow-hidden flex items-center justify-center aspect-square shadow-blue-200/20 group-hover/img:shadow-blue-300/40 transition duration-500 border-2 border-transparent group-hover/img:border-blue-600/10">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-blue-600/5 rounded-full blur-3xl transform translate-x-32 -translate-y-32"></div>
                            {product.image ? (
                                <img src={product.image} alt={product.name} className="w-full h-full object-contain mix-blend-multiply drop-shadow-2xl transform group-hover/img:scale-110 transition duration-700" />
                            ) : (
                                <Droplet className="w-48 h-48 text-blue-200 animate-pulse" />
                            )}
                            <div className="absolute bottom-10 left-10 flex space-x-4">
                                <div className="px-6 py-2 bg-slate-900 text-white rounded-full text-[10px] font-extrabold uppercase tracking-widest flex items-center shadow-2xl">
                    ...
                    <div className="flex flex-col">
                        <div className="mb-12">
                            <div className="flex items-center space-x-2 text-blue-600 font-bold text-xs uppercase tracking-widest mb-6">
                                <ShieldCheck className="w-4 h-4" />
                                <span>Trusted Multi-Stage Filtration</span>
                            </div>
                            <h1 className="text-5xl font-extrabold text-slate-900 tracking-tight leading-tight mb-6 group-hover:text-blue-600 transition duration-300">{product.name}</h1>
                            <div className="flex items-center space-x-8 mb-10">
                                <div className="flex items-center space-x-1 text-amber-400">
                                    {[1, 2, 3, 4, 5].map(s => <Star key={s} className="w-5 h-5 fill-current" />)}
                                </div>
                                <span className="text-sm font-bold text-slate-400 uppercase tracking-widest border-l border-slate-200 pl-8">(4.9/5 from 340+ Reviews)</span>
                            </div>
                        </div>

                        <div className="bg-white p-12 rounded-[3.5rem] shadow-xl border border-slate-100 mb-12 relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-8 flex flex-col items-end">
                                <span className="text-slate-400 font-bold line-through text-xl leading-none italic mb-1">Was ₹{(product.price * 1.25).toFixed(0)}</span>
                                <span className="text-5xl font-extrabold text-blue-600 tracking-tighter leading-none">₹{product.price}</span>
                            </div>

                            <div className="space-y-8 relative z-10 max-w-[70%]">
                                <div>
                                    <h3 className="text-xs font-extra-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center"><Info className="w-4 h-4 mr-2" /> Product Specification</h3>
                                ...
                                <div className="grid grid-cols-2 gap-8 pt-8 border-t border-slate-50">
                                    <div className="flex items-center space-x-4 group/spec">
                                        <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-500 group-hover/spec:bg-blue-600 group-hover/spec:text-white transition group-hover/spec:rotate-6">
                                            <HeartPulse className="w-6 h-6" />
                                        </div>
                                        <div>
                                            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Purification</span>
                                            <span className="text-sm font-bold text-slate-700">RO+UV+UF+MTDS</span>
                                        </div>
                                    </div>
                                    <div className="flex items-center space-x-4 group/spec">
                                        <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-500 group-hover/spec:bg-blue-600 group-hover/spec:text-white transition group-hover/spec:rotate-6">
                                        ...
                                        <div>
                                            <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Capacity</span>
                                            <span className="text-sm font-bold text-slate-700">12 Liters/Hour</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-wrap items-center gap-8">
                            <button
                                ...
                                className="flex-1 min-w-[240px] flex justify-center items-center py-6 px-12 bg-blue-600 text-white font-extrabold text-xl rounded-3xl shadow-2xl shadow-blue-200 hover:bg-blue-700 transform active:scale-95 transition-all group/buy"
                            >
                                <ShoppingBag className="w-6 h-6 mr-4 group-hover/buy:rotate-12 transition" />
                                <span>Acquire This System</span>
                            </button>

                            <div className="flex items-center space-x-8 px-10 py-6 bg-slate-900 rounded-3xl shadow-2xl">
                                <div className="flex flex-col items-center">
                                    <CheckCircle className="w-6 h-6 text-green-500 mb-1" />
                                    <span className="text-[10px] font-bold text-white uppercase tracking-widest">WARRANTY</span>
                                </div>
                                <div className="w-px h-8 bg-white/10"></div>
                                <div className="flex flex-col items-center">
                                    <Droplet className="w-6 h-6 text-blue-400 mb-1" />
                                    <span className="text-[10px] font-bold text-white uppercase tracking-widest">CERTIFIED</span>
                                </div>
                            </div>
                        </div>

                        <p className="mt-12 text-center text-slate-400 font-bold text-xs uppercase tracking-[0.2em] italic">
                            FREE SHIPPING ON ALL INSTALLATIONS • CASH ON DELIVERY ONLY
                        </p>
                    </div>
                </div>
            </main>
